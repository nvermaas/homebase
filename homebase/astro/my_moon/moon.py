...
# construct a json object to return to the REST API
def get_phase_json(title,phase_date):
    phase = {"phase": title,
             "date": str(phase_date.strftime("%d %b %Y")),
             "time": str(phase_date.strftime("%H:%M"))}
    return phase


def get_new_moons(year):
    """Find time of phases of the new moon for a given year
    """
    newmoons = []
    ...
    while jd < end_jd:
        phase_date = truephase(k, 0.0)
        phase_date_jd = date_to_jd(phase_date)
        if (start_jd <= phase_date_jd <= end_jd):
            phase = get_phase_json('New Moon', phase_date)
            newmoons.append(phase)

        phase_date = truephase(k, 0.25)
        if (start_jd <= date_to_jd(phase_date) <= end_jd):
            phase = get_phase_json('First Quarter', phase_date)
            newmoons.append(phase)

        phase_date = truephase(k, 0.5)
        if (start_jd <= date_to_jd(phase_date) <= end_jd):
            phase = get_phase_json('Full Moon', phase_date)
            newmoons.append(phase)

        phase_date = truephase(k, 0.75)
        if (start_jd <= date_to_jd(phase_date) <= end_jd):
            phase = get_phase_json('Last Quarter', phase_date)
            newmoons.append(phase)

        jd = jd + c.synodic_month
        k = k + 1
